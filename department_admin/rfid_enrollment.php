...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_rfid'])) {
    require_once __DIR__ . '/../api/config.php';
    $student_id = (int)$_POST['student_id'];

    mysqli_query($conn, "UPDATE enrollment_requests SET status='Cancelled' WHERE status='Pending'");

    $stmt = mysqli_prepare($conn, "INSERT INTO enrollment_requests (student_id, enrollment_type, fingerprint_id, status) VALUES (?, 'rfid', NULL, 'Pending')");
    mysqli_stmt_bind_param($stmt, "i", $student_id);
    if (mysqli_stmt_execute($stmt)) {
        $message = "RFID enrollment request sent to ESP32. Tap card on device."; $message_class = "success";
        log_activity($conn, $deptAdmin['id'], 'department_admin', 'RFID Enrollment', "RFID for student ID $student_id");
    } else {
...
<div class="page-title"><h1>RFID Enrollment</h1><p>Send RFID enrollment requests to ESP32</p></div>
...
<div class="form-row">
<form method="POST">
<h3><i class="fas fa-id-card"></i> New RFID Request</h3>
<label>Student</label>
<select name="student_id" required>
<option value="">Select Student</option>
<?php while ($s = mysqli_fetch_assoc($students)) { ?>
<option value="<?php echo $s['id']; ?>"><?php echo htmlspecialchars($s['matric_no'] . ' - ' . $s['fullname'] . ' | RFID: ' . ($s['rfid_uid'] ?: 'None')); ?></option>
<?php } ?>
</select>
<br><br>
<button type="submit" name="send_rfid" class="btn btn-primary"><i class="fas fa-paper-plane"></i> Send to Device</button>
</form>
...
<thead><tr><th>Student</th><th>Type</th><th>RFID UID</th><th>Status</th><th>Created</th></tr></thead>
<tbody>
...
<?php } } else { ?>
<tr><td colspan="5" style="text-align:center;color:#999;padding:30px;">No requests yet</td></tr>
<?php } ?>
</tbody>
</table>
</div>
